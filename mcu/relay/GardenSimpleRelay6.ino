...
static const uint8_t RGB_LED_BRIGHTNESS = 16;
...
.relay-row{display:flex;justify-content:space-between;align-items:center;gap:10px}
.relay-meta{display:flex;align-items:center;gap:8px}
.relay-controls{margin-left:auto;justify-content:flex-end}
.relay-card.active-zone{background:rgba(var(--zone-color-rgb,62,184,255),0.2)!important;box-shadow:inset 0 0 0 1px rgba(var(--zone-color-rgb,62,184,255),0.45)}
...
function addScheduleRow(v){const rows=document.getElementById('adminSchedRows');const row=document.createElement('div');row.className='actions';const z=Number(v&&v.zone||1);const t=(v&&v.timeValue)||'06:00';const m=Number(v&&v.runMinutes||10);row.innerHTML=`<input type=number min=1 max=5 value='${z}' title='Zone'><input type=time style='max-width:84px' value='${t}' title='Start'><input type=number min=1 max=240 style='max-width:62px' value='${m}' title='Minutes'><input type='hidden' value='on' title='Enabled'><button class='danger' onclick="this.parentElement.remove()">Delete</button>`;rows.appendChild(row)}
...
async function refresh(forceScheduleRedraw){const s=await (await fetch('/api/state',{cache:'no-store'})).json();hydrateConfig(s);meta.textContent=`${s.date} ${s.time} · WiFi ${s.homeWifiConnected?s.homeIp:s.homeWifiStatus} · Master ${s.masterValveOn?'ON':'OFF'}`;const rg=document.getElementById('relay-grid');const spigotsActive=!!(s.spigotRun&&s.spigotRun.active);const zoneCards=(s.zones||[]).map(z=>{const c=colorByChannel(s.zoneColors, zoneChannel(z));const rgb=toRgb(c);return `<li class="relay-card ${z.relayOn?'active-zone':''}" style="--zone-color-rgb:${rgb};background:rgba(${rgb},0.08);border-color:rgba(${rgb},0.35)"><div class="relay-row"><div class="relay-meta"><b>Zone ${z.zone}</b> <span class='${z.relayOn?'status-synced':'status-mismatch'}'>${z.relayOn?'ON':'OFF'}</span></div><div class="actions relay-controls"><input id="m${z.zone}" type=number min=1 max=240 value=15><button onclick="cmd('/api/manual-run?zone=${z.zone}&minutes='+encodeURIComponent(document.getElementById('m${z.zone}').value))">Run</button><button onclick="cmd('/api/relay?zone=${z.zone}&state=0')">Stop</button></div></div></li>`}).join('');const spigotCard=`<li class="relay-card"><div class="relay-row"><div class="relay-meta"><b>Spigots</b> <span class='${spigotsActive?'status-synced':'status-mismatch'}'>${spigotsActive?'ON':'OFF'}</span></div><div class="actions relay-controls"><input id="m-spigots" type=number min=1 max=240 value=15><button onclick="cmd('/api/spigots-run?minutes='+encodeURIComponent(document.getElementById('m-spigots').value))">Run</button><button onclick="cmd('/api/spigots-run?action=off')">Stop</button></div></div></li>`;rg.innerHTML=spigotCard+zoneCards;(s.zones||[]).forEach(z=>zids(zoneChannel(z)).forEach(id=>{const e=document.getElementById(id);if(!e)return;const c=colorByChannel(s.zoneColors, zoneChannel(z));const rgb=toRgb(c);e.style.fill=`rgba(${rgb},${z.relayOn?0.55:0.2})`;e.style.stroke=`rgba(${rgb},0.9)`;e.classList.toggle('zone-active',!!z.relayOn)}));const schedules=normalizeSchedules(s.dailySchedules||[]);const nextScheduleKey=scheduleKey(schedules);const shouldRedrawSchedules=forceScheduleRedraw||nextScheduleKey!==lastScheduleKey;if(shouldRedrawSchedules){document.getElementById('timeline').innerHTML=renderTimeline(buildTimelineRows(schedules), s.zoneColors||[]);if(forceScheduleRedraw||!scheduleEditorBusy()){const rows=document.getElementById('adminSchedRows');rows.innerHTML='';(s.dailySchedules||[]).forEach(addScheduleRow);}lastScheduleKey=nextScheduleKey;}}
