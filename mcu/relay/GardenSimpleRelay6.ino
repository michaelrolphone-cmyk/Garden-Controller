...
<section class="panel"><h1>Castle Hills Garden Manager (Firmware Local)</h1><p id="meta">Loading state...</p><p><a href="/">Mobile View</a></p><div class="actions"><button onclick="syncTime()">Sync Phone Time</button><button onclick="fetch('/api/buzzer-test')">Test Buzzer</button><button onclick="cmd('/api/alloff')">Turn Off All Relays</button><button onclick="const m=prompt('Run spigots for how many minutes?','15');if(m)cmd('/api/spigots-run?minutes='+encodeURIComponent(m))">Run Spigots</button><button onclick="cmd('/api/spigots-run?action=off')">Stop Spigots</button><button onclick="testRemote()">Test Remote API</button><button onclick="factoryReset()" style="background:#9a2b2b;color:#fff">Factory Reset</button></div></section>
...
<section class="panel"><h2>Device & Connectivity Settings</h2><p>Restore and edit mission-critical firmware settings directly on-device.</p><div class="grid"><label>AP SSID<input id="apSsid" autocomplete="off"></label><label>AP Password<input id="apPass" autocomplete="off"></label><label>Home WiFi SSID<input id="staSsid" autocomplete="off"></label><label>Home WiFi Password<input id="staPass" autocomplete="off"></label><label>Remote API Base URL<input id="remoteApiBase" autocomplete="off"></label><label>Device ID / local metadata<input id="remoteDeviceId" autocomplete="off"></label><label>API Token<input id="remoteApiKey" autocomplete="off"></label><label><input type="checkbox" id="remoteEnabled"> Remote API enabled</label><label>Garden latitude<input id="gardenLatitude" type="number" step="0.000001"></label><label>Garden longitude<input id="gardenLongitude" type="number" step="0.000001"></label><label>Garden IANA timezone<input id="gardenTimeZone" autocomplete="off"></label><label>ESP32 POSIX timezone<input id="gardenPosixTimeZone" autocomplete="off"></label></div><div class="actions"><button onclick="saveConfig()">Save Device Settings</button></div></section>
...
function addScheduleRow(v){const rows=document.getElementById('adminSchedRows');const row=document.createElement('div');row.className='actions';const z=Number(v&&v.zone||1);const t=(v&&v.timeValue)||'06:00';const m=Number(v&&v.runMinutes||10);const en=v&&v.enabled===false?'off':'on';row.innerHTML=`<input type=number min=1 max=5 value='${z}' title='Zone'><input type=time value='${t}' title='Start'><input type=number min=1 max=240 value='${m}' title='Minutes'><select title='Enabled'><option value='on'>On</option><option value='off'>Off</option></select><button onclick="this.parentElement.remove()">Delete</button>`;row.querySelector('select').value=en;rows.appendChild(row)}
